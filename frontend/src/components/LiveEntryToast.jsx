...
function LiveEntryToast({ entry, onProfileClick, onDismiss }) {
  ...
  return (
    <div 
      className={`live-entry-toast ${isClosing ? 'slide-out' : 'slide-in'} ${isNoPass ? 'nopass-alert' : isExpired ? 'expired-alert' : isFrozen ? 'frozen-alert' : isExpiringSoon ? 'expiring-alert' : 'active-entry'}`}
      ...
      <div className="toast-body-info">
        <div className="toast-top-meta">
          <span className="toast-verified-tag">
            {isNoPass ? '🚫 NO MEMBERSHIP PASS' : isExpired ? '🚨 PASS EXPIRED' : isFrozen ? '❄️ PASS FROZEN' : isExpiringSoon ? '⚠️ EXPIRING SOON' : '✨ ENTRY VERIFIED'}
          </span>
